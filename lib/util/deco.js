 ...
 * つまり、戻り値は
 *   [ [ 0スロのスキルの組み合わせ ], [ 1スロの ], [ 2スロの ], [ 3スロの ] ]
 * という形式。
 *
 * e.g.
 *     skillsCombBySlot(decos, [ '攻撃', '斬れ味' ]);
 *     => [ [],
 *          [ { '攻撃': 1, '防御': -1 }, { '斬れ味': 1, '匠': -1 } ],
 *          [ { '攻撃': 2, '防御': -2 },
 *            { '攻撃': 1, '防御': -1, '斬れ味': 1, '匠': -1 },
 ...
 *            { '斬れ味': 4, '匠': -2 } ] ]
 */
function skillsCombBySlot(decos, skilltrees) {
    if (skilltrees == null || skilltrees.length === 0) return [];

    let decosCombBySlot = combs(decos, skilltrees);
    let bySlot = [];

    for (let i = 0, ilen = decosCombBySlot.length; i < ilen; ++i) {
        let decosComb = decosCombBySlot[i];
        let skillsComb = [];
        for (let j = 0, jlen = decosComb.length; j < jlen; ++j) {
            let decos = decosComb[j];
            let list = [];
            for (let k = 0, klen = decos.length; k < klen; ++k) {
                list.push(decos[k].skills);
            }
            skillsComb.push(util.join(list));
        }
        bySlot.push(skillsComb);
    }

    return bySlot;
}
exports.skillsCombBySlot = skillsCombBySlot;
