 ...
 *     // equips はスキルが発動する装備の組み合わせ
 *     n.normalize([ '斬れ味レベル+1', '砥石使用高速化' ], equips);
 *     => { head: [
...
class Normalizer {
    ...
    normalize(skillnames, equips) {
        ...
        for (let i = 0, len = parts.length; i < len; ++i) {
            let part = parts[i];
            let equip = equips[part];

            let bulks;
            bulks = this._normalize1(decosCombBySlot, equip);
...
function makeBulk(decos, slot, skills) {
    ...
    for (let i = 0, len = decos.length; i < len; ++i) {
        deconames.push(decos[i].name);
        skillsComb.push(decos[i].skills);
    }

    bulk.decos = deconames;
