...
describe('mh4g/equip-normalizer', () => {
    ...
    describe('normalize: selected equips', () => {
        it('should normalize if contain a fixed equip', () => {
            ...
            let exp = [
                { skills: { '攻撃': 1, '斬れ味': -2 }, equips: [ 'アカムトウルンテ' ] },
                { skills: { '攻撃': 0, '斬れ味': -1 }, equips: [ 'アカムトウルンテ' ] }
            ];
        ...
        it('should normalize if contain selected equips', () => {
            ...
            myapp.data.equips.body = [
                { name: 'ブナハＳスーツ', slot: 0,
                  skills: { '攻撃': -2, ' 特殊攻撃': 2, '納刀': 2, '麻痺': 1 } },
                { name: 'リベリオンメイル', slot: 1,
                  skills: { '刀匠': 3, '怒': 1, '攻撃': -4 } },
                { name: 'アカムトウルンテ', slot: 1,
                  skills: { '匠': 2, '達人': 3, '聴覚保護': 1, '斬れ味': -2 } }
            ];
            context.init(myapp.data);
    ...
    describe('normalize: charm', () => {
        it('should normalize if contain charms', () => {
            ...
            let exp = [
                { skills: { '匠': 0, '攻撃': 0, '聴覚保護': 3 },
                  equips: [ '龍の護石(スロ3,痛撃+4)' ] },
                { skills: { '匠': 0, '攻撃': 1, '聴覚保護': 2 },
                  equips: [ '龍の護石(スロ3,痛撃+4)' ] },
                { skills: { '匠': 0, '攻撃': 3, '聴覚保護': 1 },
                  equips: [ '龍の護石(スロ3,痛撃+4)' ] },
                { skills: { '匠': 0, '攻撃': 4, '聴覚保護': 0 },
                  equips: [ '龍の護石(スロ3,痛撃+4)' ] },
                { skills: { '匠': 4, '攻撃': 0, '聴覚保護': 3 },
                  equips: [ '龍の護石(スロ3,匠+4,氷耐性-5)' ] },
                { skills: { '匠': 4, '攻撃': 1, '聴覚保護': 2 },
                  equips: [ '龍の護石(スロ3,匠+4,氷耐性-5)' ] },
                { skills: { '匠': 4, '攻撃': 3, '聴覚保護': 1 },
                  equips: [ '龍の護石(スロ3,匠+4,氷耐性-5)' ] },
                { skills: { '匠': 4, '攻撃': 4, '聴覚保護': 0 },
                  equips: [ '龍の護石(スロ3,匠+4,氷耐性-5)' ] },
                { skills: { '匠': 0, '攻撃': 9, '聴覚保護': 0 },
    ...
    describe('normalize: dig', () => {
        it('should normalize if contain dig equips', () => {
            ...
            let got = bulksSet.weapon;
            let exp = [
                { skills: { '刀匠': 0, '溜め短縮': 0, '痛撃': 0, '聴覚保護': 0 },
                  equips: [
                      '発掘(状耐+2)','発掘(状耐+3)','発掘(状耐+4)',
                      '発掘(回避+3)','発掘(回避+4)','発掘(回避+5)',
                      '発掘(強欲+3)','発掘(強欲+4)','発掘(強欲+6)',
                      '発掘(護収+3)','発掘(護収+4)','発掘(護収+6)' ] },
                { skills: { '刀匠': 2, '溜め短縮': 0, '痛撃': 0, '聴覚保護': 0 },
                  equips: [ '発掘(刀匠+2)' ] },
                { skills: { '刀匠': 3, '溜め短縮': 0, '痛撃': 0, '聴覚保護': 0 },
                  equips: [ '発掘(刀匠+3)' ] },
                { skills: { '刀匠': 4, '溜め短縮': 0, '痛撃': 0, '聴覚保護': 0 },
                  equips: [ '発掘(刀匠+4)' ] }
            ];
            assert.deepEqual(got, exp);
        });
